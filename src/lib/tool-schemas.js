...
export const TOOLS = [
  ...
  { name: 'clear_behavior', description: 'Remove a behaviour reminder (operation + scope + match), or ALL of them if both operation and scope are omitted. A scope with no operation targets operation "receive". Sub-peers pass as + secret.',
    inputSchema: { type: 'object', properties: {
      operation: { type: 'string', enum: ['receive', 'send', 'publish', 'claim_topic', 'release_topic', 'subscribe', 'allow_project', 'revoke_project', 'request_project_access'], description: 'the operation to clear (default "receive" when a scope is given; legacy alias "deliver" also accepted); omit with scope to clear every reminder' },
