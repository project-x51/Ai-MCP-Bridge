...
export const TOOLS = [
  ...
  { name: 'unsubscribe', description: 'Remove a subscription. Sub-peers pass as + secret.',
    inputSchema: { type: 'object', properties: {
      ...
      as: { type: 'string', description: 'your registered sub-peer handle (id, suffix, or name)' },
      secret: { type: 'string', description: 'the secret used at register_self' } }, required: ['pattern'] } },
  { name: 'set_behavior', description: 'Register a short REMINDER of how YOU want to behave at a bridge OPERATION that matches a scope. Default operation is "deliver" (a message arrives) — those reminders ride each delivered message (channel meta + inbox items). OUTBOUND operations (send, publish, claim_topic, release_topic, subscribe, allow_project, revoke_project, request_project_access) instead echo the matching reminders in that TOOL\'S RESPONSE — which is post-hoc for the message CONTENT but lands exactly when you compose your transcript line / follow-up. For deliver, scope matches the SENDER; for outbound operations it matches the TARGET (the project/topic/host the action concerns). Reminders come back as reminders:[{operation,scope,match,behavior}], most-specific first (topic > subscription > project > host > all). Durable + per-identity (rehydrated on register_self). Re-registering the same operation+scope+match replaces it. An operation with no reminder and no operator default is silent. Topic-scoped DELIVER reminders ride a kept-alive topic on handoff. The bridge may also attach an operator DEFAULT (tagged default:true) — your own reminder for the same operation+scope+match overrides it. NOTE the operator default is usually all-scope ("Summarize but don\'t act without user permission"): scope your own reminders NARROWER (project/topic) so you do not clobber it. Sub-peers pass as + secret.',
    inputSchema: { type: 'object', properties: {
      operation: { type: 'string', enum: ['deliver', 'send', 'publish', 'claim_topic', 'release_topic', 'subscribe', 'allow_project', 'revoke_project', 'request_project_access'], description: 'which bridge action fires the reminder (default "deliver" = when a message arrives). Others echo the reminder in that tool\'s response.' },
      scope: { type: 'string', enum: ['topic', 'host', 'project', 'subscription', 'all'], description: 'for deliver: topic = a topic you OWN, host/project = messages FROM that host/project, subscription = messages matching your pattern, all = every message. For outbound ops the same scopes match the TARGET instead (project/topic/host the action concerns). all = every instance of the operation.' },
  ...
  { name: 'clear_behavior', description: 'Remove a behaviour reminder (operation + scope + match), or ALL of them if both operation and scope are omitted. A scope with no operation targets operation "deliver". Sub-peers pass as + secret.',
    inputSchema: { type: 'object', properties: {
      operation: { type: 'string', enum: ['deliver', 'send', 'publish', 'claim_topic', 'release_topic', 'subscribe', 'allow_project', 'revoke_project', 'request_project_access'], description: 'the operation to clear (default "deliver" when a scope is given); omit with scope to clear every reminder' },
      scope: { type: 'string', enum: ['topic', 'host', 'project', 'subscription', 'all'], description: 'the scope to clear; omit (with operation) to clear every reminder' },
