...
await sleep(200)
// §21: a lost/wrong secret is still rejected as before, but now carries a hint pointing at recover_secret (vault on)
const taken = await call(B, 'register_self', { name: 'Forgetful', secret: 'WRONG-lost-it', project: 'ops' })
check('lost-secret re-register -> name-taken WITH recover hint (vault on)', taken.code === 'name-taken' && taken.recoverable === true && /recover_secret/.test(taken.hint || ''), JSON.stringify(taken))
const badTool = await call(B, 'claim_topic', { topic: 'x', as: 'Forgetful', secret: 'WRONG-lost-it' })
check('lost-secret tool call -> bad-secret WITH recover hint (vault on)', badTool.code === 'bad-secret' && badTool.recoverable === true && /recover_secret/.test(badTool.hint || ''), JSON.stringify(badTool))
...
const takenNoVault = await call(B, 'register_self', { name: 'Plain', secret: 'WRONG', project: 'ops' })
check('name-taken carries NO recover hint when there is no vault', takenNoVault.code === 'name-taken' && !takenNoVault.recoverable && !takenNoVault.hint, JSON.stringify(takenNoVault))
