...
// id FORM is owned by test_stable_ids_live (which pins the mode); asserting a shape here would make this
// suite depend on the operator's config.json. What matters to THIS suite is that Owner registered.
check('Owner registered live', ow.ok === true && typeof ow.peer_id === 'string' && ow.peer_id.length > 0, ow.peer_id)
