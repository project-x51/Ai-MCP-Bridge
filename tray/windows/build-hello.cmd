@echo off
rem Build HelloConfirm.exe (the Windows Hello confirmation helper used by the bridge `hello` authorizer
rem facet) with the in-box .NET Framework compiler + the OS WinRT metadata. Needs .NET Framework 4.x.
rem No (...) command-grouping blocks: a "(x86)" path inside one would break the parser.
setlocal
set "DIR=%~dp0"
...
"%CSC%" /nologo /target:exe /out:"%DIR%HelloConfirm.exe" /r:System.Core.dll /r:"%FACADE%" /r:"%WINRT%" /r:"%FOUND%" /r:"%SEC%" "%DIR%HelloConfirm.cs"
